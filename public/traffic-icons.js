(function (global) {
    ...
    const turnLeft = `
      ${ring}
      <path d="M66 76 L66 48 Q66 34 52 34 L42 34" fill="none" stroke="${white}" stroke-width="${SHAFT}" stroke-linecap="butt" stroke-linejoin="round"/>
      <polygon points="22,34 44,16 44,52" fill="${white}"/>
    `;

    const turnRight = `
      ${ring}
      <path d="M34 76 L34 48 Q34 34 48 34 L58 34" fill="none" stroke="${white}" stroke-width="${SHAFT}" stroke-linecap="butt" stroke-linejoin="round"/>
      <polygon points="78,34 56,16 56,52" fill="${white}"/>
    `;

    /**
     * 206 可選擇之應遵方向：左或右
     * 箭頭頭部縮小約 8px（原 18,42 40,24 40,60 → 24,42 38,28 38,56）
     */
    const leftRight = `
      ${ring}
      <path d="M50 76 L50 54 Q50 42 38 42 L36 42" fill="none" stroke="${white}" stroke-width="${SHAFT}" stroke-linecap="butt" stroke-linejoin="round"/>
      <polygon points="24,42 38,28 38,56" fill="${white}"/>
      <path d="M50 54 Q50 42 62 42 L64 42" fill="none" stroke="${white}" stroke-width="${SHAFT}" stroke-linecap="butt" stroke-linejoin="round"/>
    ...
    const straightLeft = `
      ...
      <path d="M54 58 L44 58" fill="none" stroke="${white}" stroke-width="${SHAFT}" stroke-linecap="butt" stroke-linejoin="round"/>
    ...
    const straightRight = `
      ...
      <path d="M46 58 L56 58" fill="none" stroke="${white}" stroke-width="${SHAFT}" stroke-linecap="butt" stroke-linejoin="round"/>
    ...
    const roundabout = `
      ${ring}
      <path d="M 53.8,28.3 A 22,22 0 0,1 70.7,57.5" fill="none" stroke="${white}" stroke-width="6" stroke-linecap="butt"/>
      <polygon points="67.6,66.0 77.3,59.9 64.1,55.1" fill="${white}"/>
      <path d="M 66.9,64.1 A 22,22 0 0,1 33.1,64.1" fill="none" stroke="${white}" stroke-width="6" stroke-linecap="butt"/>
      <polygon points="27.4,57.2 27.8,68.6 38.5,59.6" fill="${white}"/>
      <path d="M 29.3,57.5 A 22,22 0 0,1 46.2,28.3" fill="none" stroke="${white}" stroke-width="6" stroke-linecap="butt"/>
      <polygon points="55.0,26.8 45.0,21.4 47.4,35.2" fill="${white}"/>
    `;

    const busLane = `
      ${ring}
      <rect x="18" y="36" width="64" height="28" rx="5" ry="5" fill="none" stroke="${white}" stroke-width="5"/>
    ...
    const motorcycleLane = `
      ...
      <path d="M32 67 L45 54 L56 67 L68 67" fill="none" stroke="${white}" stroke-width="5" stroke-linecap="round" stroke-linejoin="round"/>
      <path d="M45 54 L52 43 L62 49" fill="none" stroke="${white}" stroke-width="5" stroke-linecap="round" stroke-linejoin="round"/>
      <path d="M52 43 L42 39" fill="none" stroke="${white}" stroke-width="5" stroke-linecap="round"/>
      <circle cx="50" cy="31" r="6" fill="${white}"/>
      <path d="M50 37 L48 48 L40 55" fill="none" stroke="${white}" stroke-width="5" stroke-linecap="round" stroke-linejoin="round"/>
    `;
    ...
    const straightLeftRight = `
      ...
      <rect x="${50 - HALF_SHAFT}" y="38" width="${SHAFT}" height="40" fill="${white}"/>
      <polygon points="50,20 32,42 68,42" fill="${white}"/>
      <path d="M50 60 L36 60" fill="none" stroke="${white}" stroke-width="${SHAFT}" stroke-linecap="butt"/>
      <polygon points="18,60 40,42 40,78" fill="${white}"/>
      <path d="M50 60 L64 60" fill="none" stroke="${white}" stroke-width="${SHAFT}" stroke-linecap="butt"/>
      <polygon points="82,60 60,42 60,78" fill="${white}"/>
    `;

    const uTurn = `
      ${ring}
      <path d="M68 76 L68 50 A18 18 0 0 0 32 50 L32 64" fill="none" stroke="${white}" stroke-width="${SHAFT}" stroke-linecap="round" stroke-linejoin="round"/>
      <polygon points="32,80 16,60 48,60" fill="${white}"/>
    `;

    const uTurnRight = `
      ${ring}
      <path d="M32 76 L32 50 A18 18 0 0 1 68 50 L68 64" fill="none" stroke="${white}" stroke-width="${SHAFT}" stroke-linecap="round" stroke-linejoin="round"/>
    ...
    global.TRAFFIC_ICON_SVGS = Object.fromEntries(Object.entries(icons).map(([id, content]) => [
        id,
        `<svg viewBox="0 0 100 100" width="100%" height="100%" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="mandatory traffic sign ${id}">${content}</svg>`
    ]));
})(window);
